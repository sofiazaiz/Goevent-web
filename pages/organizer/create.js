...
export default function CreateEventPage() {
  ...
  const [recurrenceWeekday, setRecurrenceWeekday] = useState(null);
  ...
  const handleSubmit = async () => {
    ...
    if (recurrenceType === "weekly" && recurrenceWeekday !== null) {
        ...
        {recurrenceType === "weekly" && (
          <div className="flex flex-wrap gap-2 mt-3">
            {WEEKDAYS.map((d) => (
              <button
                key={d.value}
                onClick={() => setRecurrenceWeekday(d.value)}
                className={`weekday ${
                  recurrenceWeekday === d.value ? "weekday-active" : ""
                }`}
              >
                {d.label}
              </button>
